...
X86DAGToDAGISel::X86DAGToDAGISel(
    X86TargetMachine &tm,
    CodeGenOpt::Level OL)
  : DAGMatcher(tm, new SelectionDAG(tm, OL), OL)
  , X86DAGMatcher(tm, OL)
...
bool X86DAGToDAGISel::runOnMachineFunction(MachineFunction &MF) {
...
void X86DAGMatcher::getAddressOperands(X86ISelAddressMode &AM, const SDLoc &DL,
...
//
bool X86DAGMatcher::shouldAvoidImmediateInstFormsForSize(SDNode *N) const {
  uint32_t UseCount = 0;
...
bool X86DAGMatcher::isUnneededShiftMask(SDNode *N, unsigned Width) const {
...
bool X86DAGMatcher::useNonTemporalLoad(LoadSDNode *N) const {
  if (!N->isNonTemporal())
    return false;
...
// for us.
bool X86DAGMatcher::isMaskZeroExtended(SDNode *N) const {
...
bool
X86DAGMatcher::IsProfitableToFold(SDValue N, SDNode *U, SDNode *Root) const {
...
bool X86DAGMatcher::isProfitableToFormMaskedOp(SDNode *N) const {
  assert(
...
bool X86DAGMatcher::tryOptimizeRem8Extend(SDNode *N) {
...
bool X86DAGMatcher::foldOffsetIntoAddress(uint64_t Offset,
...
bool X86DAGMatcher::matchLoadInAddress(LoadSDNode *N, X86ISelAddressMode &AM){
...
bool X86DAGMatcher::matchWrapper(SDValue N, X86ISelAddressMode &AM) {
...
bool X86DAGMatcher::matchAddress(SDValue N, X86ISelAddressMode &AM) {
...
bool X86DAGMatcher::matchAdd(SDValue &N, X86ISelAddressMode &AM,
                               unsigned Depth) {
...
bool X86DAGMatcher::matchAddressRecursively(SDValue N, X86ISelAddressMode &AM,
                                              unsigned Depth) {
  SDLoc dl(N);
  LLVM_DEBUG({
    dbgs() << "MatchAddress: ";
...
bool X86DAGMatcher::matchAddressBase(SDValue N, X86ISelAddressMode &AM) {
...
/// been handled.
bool X86DAGMatcher::matchVectorAddress(SDValue N, X86ISelAddressMode &AM) {
...
bool X86DAGMatcher::selectVectorAddr(MemSDNode *Parent, SDValue BasePtr,
...
bool X86DAGMatcher::selectAddr(SDNode *Parent, SDValue N, SDValue &Base,
...
bool X86DAGMatcher::selectMOV64Imm32(SDValue N, SDValue &Imm) {
...
bool X86DAGMatcher::selectLEA64_32Addr(SDValue N, SDValue &Base,
                                         SDValue &Scale, SDValue &Index,
                                         SDValue &Disp, SDValue &Segment) {
...
bool X86DAGMatcher::selectLEAAddr(SDValue N,
                                    SDValue &Base, SDValue &Scale,
                                    SDValue &Index, SDValue &Disp,
                                    SDValue &Segment) {
  X86ISelAddressMode AM;
...
bool X86DAGMatcher::selectTLSADDRAddr(SDValue N, SDValue &Base,
                                        SDValue &Scale, SDValue &Index,
                                        SDValue &Disp, SDValue &Segment) {
...
bool X86DAGMatcher::selectRelocImm(SDValue N, SDValue &Op) {
...
bool X86DAGMatcher::tryFoldLoad(SDNode *Root, SDNode *P, SDValue N,
                                  SDValue &Base, SDValue &Scale,
                                  SDValue &Index, SDValue &Disp,
                                  SDValue &Segment) {
...
bool X86DAGMatcher::tryFoldBroadcast(SDNode *Root, SDNode *P, SDValue N,
                                       SDValue &Base, SDValue &Scale,
                                       SDValue &Index, SDValue &Disp,
                                       SDValue &Segment) {
  assert(Root && P && "Unknown root/parent nodes");
...
/// if necessary.
SDNode *X86DAGMatcher::getGlobalBaseReg() {
  unsigned GlobalBaseReg = getInstrInfo()->getGlobalBaseReg(MF);
  auto &DL = MF->getDataLayout();
  return CurDAG->getRegister(GlobalBaseReg, TLI->getPointerTy(DL)).getNode();
}

bool X86DAGMatcher::isSExtAbsoluteSymbolRef(unsigned Width, SDNode *N) const {
  if (N->getOpcode() == ISD::TRUNCATE)
...
/// other than ZF.
bool X86DAGMatcher::onlyUsesZeroFlag(SDValue Flags) const {
...
/// flag to be accurate.
bool X86DAGMatcher::hasNoSignFlagUses(SDValue Flags) const {
  // Examine each user of the node.
  for (SDNode::use_iterator UI = Flags->use_begin(), UE = Flags->use_end();
         UI != UE; ++UI) {
    // Only check things that use the flags.
...
/// CF flag to be accurate.
 bool X86DAGMatcher::hasNoCarryFlagUses(SDValue Flags) const {
  // Examine each user of the node.
  for (SDNode::use_iterator UI = Flags->use_begin(), UE = Flags->use_end();
         UI != UE; ++UI) {
    // Only check things that use the flags.
...
// here.
bool X86DAGMatcher::foldLoadStoreIntoMemOperand(SDNode *Node) {
...
//   c) x &  (-1 >> (32 - y))
//   d) x << (32 - y) >> (32 - y)
bool X86DAGMatcher::matchBitExtract(SDNode *Node) {
  assert(
...
MachineSDNode *X86DAGMatcher::matchBEXTRFromAndImm(SDNode *Node) {
  MVT NVT = Node->getSimpleValueType(0);
  SDLoc dl(Node);
...
MachineSDNode *X86DAGMatcher::emitPCMPISTR(unsigned ROpc, unsigned MOpc,
...
MachineSDNode *X86DAGMatcher::emitPCMPESTR(unsigned ROpc, unsigned MOpc,
                                             bool MayFoldLoad, const SDLoc &dl,
                                             MVT VT, SDNode *Node,
                                             SDValue &InFlag) {
  SDValue N0 = Node->getOperand(0);
...
bool X86DAGMatcher::tryShiftAmountMod(SDNode *N) {
  ...
  // This function is called after X86DAGMatcher::matchBitExtract(),
...
bool X86DAGMatcher::tryShrinkShlLogicImm(SDNode *N) {
...
bool X86DAGMatcher::matchVPTERNLOG(SDNode *Root, SDNode *ParentA,
...
bool X86DAGMatcher::tryVPTERNLOG(SDNode *N) {
  MVT NVT = N->getSimpleValueType(0);
...
bool X86DAGMatcher::shrinkAndImmediate(SDNode *And) {
...
bool X86DAGMatcher::tryVPTESTM(SDNode *Root, SDValue Setcc,
                                 SDValue InMask) {
...
// into vpternlog.
bool X86DAGMatcher::tryMatchBitSelect(SDNode *N) {
...
void X86DAGMatcher::Select(SDNode *Node) {
  MVT NVT = Node->getSimpleValueType(0);
...
#define GET_DAGISEL_BODY X86DAGMatcher
